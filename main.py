...
sheet.clear()
set_with_dataframe(worksheet=sheet, dataframe=df_merged, include_index=False, include_column_header=True, resize=True)
...
for index, row in current_week_games.iterrows():
		...
		querystring_player_stats = {"gameID": current_week_game_id, "fantasyPoints": "true", "twoPointConversions": "2",
					   ...
					   "rushTD": "6", "fumbles": "-2", "receivingYards": ".1", "receivingTD": "6", "targets": "0",
...
#drop dupe columns
df_merged['player_id'] = df_merged[['replayer_id','rplayer_id','pplayer_id','kplayer_id','fplayer_id','fanplayer_id','team_player_id']].bfill(axis=1).iloc[:, 0]
df_merged=df_merged.drop(columns=['replayer_id','rplayer_id','pplayer_id','kplayer_id','fplayer_id','fanplayer_id','team_player_id'])
...
df_merged['team'] = df_merged[['reteam','rteam','pteam','kteam','fteam','fanteam','teamname']].bfill(axis=1).iloc[:, 0]
...
df_merged = df_merged[['player_id', 'name', 'team', 'position', 'recyds', 'rectd','rushyds','rushtd','passyds','passtd','int','kicking_pts','fumbles','passcmp', 'sacks', 'defensiveints','fantasy_points','dst_td', 'turnovers']]
...
df_merged = df_merged.dropna(axis=0, how='all',subset=['recyds', 'rectd','rushyds','rushtd','passyds','passtd','int','kicking_pts','fumbles','passcmp', 'sacks', 'defensiveints','fantasy_points','dst_td', 'turnovers'])
